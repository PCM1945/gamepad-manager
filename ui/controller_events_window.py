...
from ui.stick_cartesian_map import StickCartesianMap
...
class ControllerEventsWindow(QWidget):
    ...
    def _create_status_section(self):
        ...
        # Analog sticks as circular Cartesian maps.
        sticks_layout = QHBoxLayout()

        left_container = QVBoxLayout()
        left_label = QLabel("Left Stick")
        left_label.setAlignment(Qt.AlignCenter)
        left_container.addWidget(left_label)
        self.left_stick_map = StickCartesianMap()
        left_container.addWidget(self.left_stick_map)
        self.left_stick_coords = QLabel("X: +0.00   Y: +0.00")
        self.left_stick_coords.setAlignment(Qt.AlignCenter)
        self.left_stick_coords.setFont(QFont("Consolas", 9))
        left_container.addWidget(self.left_stick_coords)

        right_container = QVBoxLayout()
        right_label = QLabel("Right Stick")
        right_label.setAlignment(Qt.AlignCenter)
        right_container.addWidget(right_label)
        self.right_stick_map = StickCartesianMap()
        right_container.addWidget(self.right_stick_map)
        self.right_stick_coords = QLabel("X: +0.00   Y: +0.00")
        self.right_stick_coords.setAlignment(Qt.AlignCenter)
        self.right_stick_coords.setFont(QFont("Consolas", 9))
        right_container.addWidget(self.right_stick_coords)
    ...
    def on_state_update(self, state):
        ...
        if "left_stick" in state:
            ...
            self.left_stick_map.set_stick_position(x, y)
            self.left_stick_coords.setText(f"X: {x:+.2f}   Y: {y:+.2f}")

        # Update right stick
        if "right_stick" in state:
            x, y = state["right_stick"]
            self.right_stick_map.set_stick_position(x, y)
            self.right_stick_coords.setText(f"X: {x:+.2f}   Y: {y:+.2f}")
